...
public final class GemBackupManager {
    ...
    public static Result createBackup(
        ...
        if (!minecraftHome.isDirectory()) {
            throw new IllegalStateException("Gem .minecraft folder does not exist: " + minecraftHome.getAbsolutePath());
        }

        String fileName = "Gem_Backup_"
    ...
    public static RestoreResult restoreBackup(
        ...
        File tempRoot = new File(launcherHome, ".gem_restore_tmp_" + stamp);
        ...
        notify(callback, "Reading Gem backup...");
        Counter counter = new Counter();
        ...
        if (!sawMinecraftData || !tempMinecraft.isDirectory() || isDirectoryEmpty(tempMinecraft)) {
            deleteRecursively(tempRoot);
            throw new IllegalStateException("Selected file does not contain Gem .minecraft data.");
        }
    ...
    private static boolean shouldSkip(@NonNull File file) {
        ...
        return lower.endsWith(".download")
                ...
                || lower.contains(".gem_restore_tmp_")
