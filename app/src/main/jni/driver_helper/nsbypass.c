...
// Gem: Android 13+ namespace creation fallback and diagnostics.
//
#include "nsbypass.h"
#include <dlfcn.h>
...
static const char* NS_TAG = "GemNSBypass";
...
static size_t gem_page_size(void) {
    long value = sysconf(_SC_PAGESIZE);
    return value > 0 ? (size_t)value : (size_t)4096;
}

static void* gem_page_start(const void* pointer) {
    const size_t page_size = gem_page_size();
...
static void* find_branch_label(void* func_start) {
    ...
    const size_t page_size = gem_page_size();
    void* func_page_start = gem_page_start(func_start);
...
static bool resolve_namespace_api(void) {
    ...
    if (hidden_create_namespace == NULL || hidden_link_namespaces == NULL) {
        ...
        if (loader_dlopen != NULL) {
            const size_t page_size = gem_page_size();
            void* loader_page_start = gem_page_start((const void*)loader_dlopen);
...
static void build_search_path(char* out, size_t out_size, const char* lib_search_path) {
    const char* native_dir = getenv("GEM_MESA_NATIVE_DIR");
    const char* alias_dir = getenv("GEM_MESA_ALIAS_DIR");
...
bool linker_ns_load(const char* lib_search_path) {
    ...
    driver_namespace = try_create_namespace_path("gem_mesa_namespace", full_path);
    if (driver_namespace == NULL) {
        driver_namespace = try_create_namespace_path("gem_mesa_namespace_min", lib_search_path);
    }

    const char* native_dir = getenv("GEM_MESA_NATIVE_DIR");
    if (driver_namespace == NULL && native_dir != NULL && native_dir[0] != '\0') {
        driver_namespace = try_create_namespace_path("gem_mesa_namespace_native", native_dir);
    }
