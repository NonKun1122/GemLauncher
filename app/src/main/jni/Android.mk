...
# Mesa direct-EGL mode for Gem Mesa/Freedreno.
#
...
# the Gem libEGL.so shim made GLFW fail before it could create the
# window/context.
...
# App-local libdrm compatibility layer for Gem/Mojo Mesa.
#
...
include $(CLEAR_VARS)
LOCAL_MODULE := drm
LOCAL_SRC_FILES := gem_libdrm_compat.c
LOCAL_LDLIBS := -ldl -llog
...
# platform libcutils.so, so Gem packages a tiny compatible subset.
LOCAL_PATH := $(HERE_PATH)
include $(CLEAR_VARS)
LOCAL_MODULE := cutils
LOCAL_SRC_FILES := gem_libcutils_compat.cpp
...
# platform libhardware.so, so Gem packages a tiny delegate/stub.
LOCAL_PATH := $(HERE_PATH)
include $(CLEAR_VARS)
LOCAL_MODULE := hardware
LOCAL_SRC_FILES := gem_libhardware_compat.cpp
LOCAL_LDLIBS := -ldl -llog
