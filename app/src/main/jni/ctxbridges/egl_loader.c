...
// Gem: Mesa/KGSL loader hardening for Android EGL platform display.
//
#include <stddef.h>
#include <stdlib.h>
#include <dlfcn.h>
...
static const char* EGL_LOADER_TAG = "GemEGLLoader";
...
static void* try_namespace_egl_once(const char* namespace_path, const char* short_name, int flags) {
     ...
     */
    const char* absolute = getenv("GEM_MESA_EGL");
...
static void* try_namespace_egl(const char* library_name, int flags) {
    if (!env_enabled("GEM_MESA_NAMESPACE")) return NULL;
     ...
     */
    void* handle = try_namespace_egl_once(getenv("GEM_MESA_NATIVE_DIR"), short_name, flags);
    ...
    handle = try_namespace_egl_once(getenv("GEM_MESA_NAMESPACE_PATH"), short_name, flags);
    if (handle != NULL) return handle;
...
void* gem_egl_get_handle(void) {
    return g_egl_handle;
}

const char* gem_egl_get_loaded_name(void) {
...
void dlsym_EGL() {
    ...
    bool mesa = env_enabled("GEM_MESA");
    ...
    if (mesa) {
        ...
        if (g_egl_handle == NULL) {
            g_egl_handle = try_namespace_egl(getenv("GEM_MESA_EGL"), flags);
        }
        if (g_egl_handle == NULL) {
            /* Fallback to the absolute APK-native Mesa EGL path. */
            g_egl_handle = try_dlopen_egl(getenv("GEM_MESA_EGL"), flags);
        }
    }
...
#undef EGLSYM
    ...
    loader_log("EGL symbols loaded from %s getPlatformDisplay=%p queryString=%p",
               gem_egl_get_loaded_name(), eglGetPlatformDisplay_p, eglQueryString_p);
}
