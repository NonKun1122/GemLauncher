...
static bool should_force_desktop_gl(void) {
    /*
     * Gem Mesa Zink/Turnip intentionally enters libpojavexec through
     ...
     */
    return env_enabled("GEM_EGL_FORCE_DESKTOP_GL")
           || env_enabled("GEM_MESA_DESKTOP_GL")
           || (env_enabled("GEM_MESA")
               && (env_is("GEM_MESA_MODE", "zink_turnip")
...
static bool should_use_safe_android_swaps(void) {
    return should_force_desktop_gl() || env_enabled("GEM_MESA_SAFE_SWAPS");
}
...
static bool try_initialize_display(EGLDisplay display, const char* label) {
    ...
    if (eglInitialize_p(display, &major, &minor) == EGL_TRUE) {
        ...
        gl_log(ANDROID_LOG_INFO, "%s eglInitialize success display=%p version=%d.%d provider=%s",
               label, display, major, minor, gem_egl_get_loaded_name());
    ...
    gl_log(ANDROID_LOG_ERROR, "%s eglInitialize failed: %04x provider=%s",
           label, err, gem_egl_get_loaded_name());
    if (env_enabled("GEM_MESA") && err == EGL_NOT_INITIALIZED) {
        gl_log(ANDROID_LOG_ERROR,
               "Mesa KGSL diagnostic: EGL_NOT_INITIALIZED. If Logcat shows /dev/dri or swrast, Mesa did not stay on the KGSL driver path. Check for GemCutils property_get(mesa.loader.driver.override)=kgsl and app-first namespace path.");
    }
    return false;
}

bool gl_init() {
    gl_log(ANDROID_LOG_INFO, "gl_init renderer=%s mesa=%s mesaMode=%s mesaDriver=%s POJAVEXEC_EGL=%s GEM_MESA_EGL=%s",
           getenv("POJAV_RENDERER") ? getenv("POJAV_RENDERER") : "<null>",
           getenv("GEM_MESA") ? getenv("GEM_MESA") : "<null>",
           getenv("POJAV_RENDERER_MESA_MODE") ? getenv("POJAV_RENDERER_MESA_MODE") : "<null>",
           getenv("GEM_MESA_DRIVER") ? getenv("GEM_MESA_DRIVER") : "<null>",
           getenv("POJAVEXEC_EGL") ? getenv("POJAVEXEC_EGL") : "<null>",
           getenv("GEM_MESA_EGL") ? getenv("GEM_MESA_EGL") : "<null>");

    dlsym_EGL();

    const bool mesa = env_enabled("GEM_MESA");
    const bool preferPlatformDisplay = mesa || env_enabled("GEM_MESA_EGL_PLATFORM_DISPLAY");
...
gl_render_window_t* gl_init_context(gl_render_window_t *share) {
    ...
    if (desktop_gl) {
        gl_log(ANDROID_LOG_INFO, "Binding EGL_OPENGL_API for Gem Mesa desktop GL");
...
void gl_swap_surface(gl_render_window_t* bundle) {
    ...
    if (bundle->newNativeSurface != NULL)
    {
        ...
        const EGLint gem_window_surface_attrs[] = {
        ...
        bundle->surface = eglCreateWindowSurface_p(
                ...
                should_use_safe_android_swaps() ? gem_window_surface_attrs : NULL);
